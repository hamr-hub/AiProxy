...
OpenClaw で AiProxy を使用するためのクイック設定ガイド。
...
# 特定のモデルでチャット
openclaw chat --model aiproxy/gemini-3-flash-preview "あなたの質問"
```
...
**Q: モデルが利用できない？**
- AiProxy Web UI でプロバイダーが設定されているか確認
- `openclaw gateway restart` を実行してゲートウェイを再起動
- `openclaw models list` を実行してモデルリストを確認

---

詳細については、[AiProxy ドキュメント](../README-JA.md) を参照してください
