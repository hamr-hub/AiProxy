...
import * as nodeApi from '../ui-modules/node-api.js';
...
export async function handleUIApiRequests(method, pathParam, req, res, currentConfig, providerPoolManager) {
    ...
    // Node registry APIs (for distributed monitoring)
    if (method === 'GET' && pathParam === '/api/nodes') {
        return await nodeApi.handleGetNodes(req, res);
    }

    if (method === 'GET' && pathParam === '/api/nodes/aggregated-monitor') {
        return await nodeApi.handleGetAggregatedMonitor(req, res);
    }

    if (method === 'POST' && pathParam === '/api/nodes/register') {
        return await nodeApi.handleNodeRegister(req, res);
    }

    if (method === 'POST' && pathParam === '/api/nodes/heartbeat') {
        return await nodeApi.handleNodeHeartbeat(req, res);
    }

    if (method === 'POST' && pathParam === '/api/nodes/unregister') {
        return await nodeApi.handleUnregisterNode(req, res);
    }
